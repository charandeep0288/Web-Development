...
fileOptions.addEventListener("click", function (e) {
  if (e.currentTarget.classList.length == 0) {
    e.currentTarget.innerText = `File
    <span>
      <span>Clear</span>
      <span>Save</span>
      <span>Open</span>
    ...
    // clear 
    allFilterOptions[0].addEventListener("click", function () {
    ...
    // open
    allFilterOptions[1].addEventListener("click", function () {
      // 1- Fetch dataObj from localStroage
    ...
    // save
    allFilterOptions[2].addEventListener("click", function () {
      localStorage.setItem("sheet", JSON.stringify() )
    });
  } else {
    e.cuurentTarget.innerHTML = `File`;
  }
});
let dataObj = {};
